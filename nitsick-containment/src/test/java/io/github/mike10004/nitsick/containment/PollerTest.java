...
import io.github.mike10004.nitsick.containment.Poller.StopReason;
import io.github.mike10004.nitsick.containment.Poller.PollOutcome;
import io.github.mike10004.nitsick.containment.Poller.PollAnswer;
import io.github.mike10004.nitsick.containment.Poller.PollAction;
...
public class PollerTest {
    ...
    public void testDoNotSleepIfAboutToTimeOut(long intervalMs, int maxPolls, StopReason stopReason, long expectedDuration, int expectedSleeps) throws Exception {
        TestSleeper sleeper = new TestSleeper();
        PollOutcome<Void> outcome = new Poller.SimplePoller(sleeper, () -> false).poll(Duration.ofMillis(intervalMs), maxPolls);
    ...
    private void testPoller(int returnTrueAfterNAttempts, int maxPollAttempts, long interval, StopReason expectedFinishReason, long expectedDuration) throws InterruptedException {
        ...
        PollOutcome<?> evaluation = poller.poll(Duration.ofMillis(interval), maxPollAttempts);
